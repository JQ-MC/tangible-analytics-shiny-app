...
infolectura_fitxer<- function(nom_xat, input, w, tmp, color_triat){
  ...
  "
    ...
    if (idioma_movil=="x" | any(!complete.cases(f)) | nrow(f) < nrow(estructura_false)| length(unique(f$usuari))>256){
        ...
        w$update(html = tagList(spin_2(),tags$br(),  tags$span(style="color:#831923; font-weight: 400; font-size:17px; background-color:white;", "Construint la teua Experiència! (+60s)")))
...
imatge_tota_conversa<- function(paraules, color_triat, tmp){
  ...
  webshot(file.path(tmp,"x.html"),file.path(ruta_particular,"Paraules.png"), delay =10, vwidth =1350, vheight=1080)
...
fer_imatges_emojis<-function(xat_emoji, coordenades , ruta_particular){
    ...
    comp<-image_resize(comp_ov, geometry_size_pixels(width=1127, height=2008))
    comp<- image_convert(comp_ov, colorspace = "sRGB")
    ...
    image_write(comp, paste0(ruta_particular, "oval.png"),'png')
    ...
    comp<-image_resize(comp_ce, geometry_size_pixels(width=1200, height=1200))
    comp<- image_convert(comp_ce, colorspace = "sRGB")
    ...
    fluidRow(
        ...
        column(7,
               ...
               selectInput(
                   ...
                   label=("Idioma de l'Analytics:"),
                   choices = c("Valencià"="val","Castellano"="cast")),
               ...
               radioGroupButtons(
                   inputId = "T",
                   label = ("Color de la imatge de les paraules:"), 
               ...
               tags$div(downloadButton("downloadData", "Tangibilitzar!",icon = shiny::icon("gift")),style="display: flex;  justify-content: center;  align-items: center; font-size: 20px;"),
...
server = function(input, output, session) {
    ...
    w <- Waiter$new(
        html = tagList(spin_2(), tags$br(), tags$span(style="color:#831923; font-weight: 400; font-size:17px; background-color:white;", "Esperant...")),
        color = transparent(.5))
    ...
    output$downloadData <- downloadHandler(
        ...
        content = function(file)
        {
            ...
            #COmençar el 
            w$show()
            w$update(html = tagList(spin_2(),tags$br(),  tags$span(style="color:#831923; font-weight: 400; font-size:17px; background-color:white;", "Llegint i estructurant el xat! (+30s)")))
            ...
            if (bo){
                ...
                w$hide()
                unlink(tmp)
                x<-zip(file, "Ooops_Problemes.html", mode="cherry-pick")
            }
